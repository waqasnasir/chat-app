...
const PersonalChat = () =>  {
    ...
    const [newchat, setNewchat] = useState({ groupname: '', email: '',name: '', message: '', date: '', type: '' });
    ...
    useEffect(()=> {
        if(user.uid < receiverId) {
            setChatId(`${user.uid}_${receiverId}`);
        } else {
            setChatId(`${receiverId}_${user.uid}`);
        }
    }, [receiverId, user])

    useEffect(() => {
        const fetchData = async () => {
            db.ref('personalchats/').orderByChild('uid').equalTo(chatId).on('value', resp => {
              setChats([]);
    ...
    const submitMessage = (e) => {
        ...
        chat.uid= chatId;
        const newMessage = db.ref('personalchats/').push();
        newMessage.set(chat);
    };
